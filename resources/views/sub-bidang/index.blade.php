...
@section('page-subtitle', 'Kelola data sub bidang PLN Icon Plus')

@section('page-actions')
    @php($actor = Auth::guard('admin')->user())
    @if(($actor && $actor->role !== 'staf'))
    <a href="{{ route('sub-bidang.create') }}" class="btn btn-modern btn-primary">
...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="sub-bidang-card">
                <div class="sub-bidang-header">
                    <h5 class="sub-bidang-title">
                        <i class="fas fa-sitemap"></i>
                        Daftar Sub Bidang PLN Icon Plus
                    </h5>
                </div>
                <div class="sub-bidang-body">
                    <!-- Filter Panel -->
                    <div class="row mb-4">
                        <div class="col-12">
                                    ...
                                    <form id="filter-form" method="GET">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                                ...
                                                <input type="text" class="form-control modern-input" name="nama_sub_bidang" value="{{ request('nama_sub_bidang') }}" placeholder="Cari nama sub bidang...">
                                            </div>
                                            <div class="col-md-3">
                        ...
                        <div class="table-responsive" id="tableContainer">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    @if((Auth::guard('admin')->user()->role ?? '') === 'super_admin')
                                    <td>
                                        <input type="checkbox" class="form-check-input modern-checkbox item-checkbox" value="{{ $sb->id }}">
                                    </td>
                                    @endif
                                    ...
                                    <td>
                                        <span class="badge modern-badge badge-bidang">{{ $sb->bidang->nama_bidang ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <small style="color: #64748b;">{{ $sb->deskripsi ?? '-' }}</small>
                                    </td>
                                    <td>
                                        <div class="action-buttons">
                                            <a href="{{ route('sub-bidang.show', $sb->id) }}" class="btn btn-modern btn-info btn-sm" title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            @if(($actor && $actor->role === 'super_admin') || ($actor && $actor->role === 'manager_bidang' && (int)$actor->bidang_id === (int)($sb->bidang_id ?? 0)))
                                            <a href="{{ route('sub-bidang.edit', $sb->id) }}" class="btn btn-modern btn-warning btn-sm" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @endif
                                            @if($actor && $actor->role === 'super_admin')
                                            <form action="{{ route('sub-bidang.destroy', $sb->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-modern btn-danger btn-sm" 
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus sub bidang ini?')" title="Hapus">
                                ...
                                <tr>
                                    <td colspan="{{ (Auth::guard('admin')->user()->role ?? '') === 'super_admin' ? '6' : '5' }}" class="text-center text-muted empty-state">
                                        <div class="empty-content">
                                            <i class="fas fa-sitemap"></i>
                                            <p>Belum ada data sub bidang. Silakan tambah data sub bidang baru.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
