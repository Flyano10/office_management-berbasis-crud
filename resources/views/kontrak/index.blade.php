...
@section('page-subtitle', 'Kelola data kontrak PLN Icon Plus')

@section('page-actions')
    <a href="{{ route('kontrak.create') }}" class="btn btn-modern btn-primary">
                                    ...
                                    <form id="filter-form" method="GET">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select modern-select" name="status_perjanjian" id="status-perjanjian-filter">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select modern-select" name="status" id="status-filter">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="date" class="form-control modern-input" name="tanggal_mulai_dari" value="{{ request('tanggal_mulai_dari') }}">
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">Tanggal Mulai Sampai</label>
                                                <input type="date" class="form-control modern-input" name="tanggal_mulai_sampai" value="{{ request('tanggal_mulai_sampai') }}">
                                            </div>
                                        </div>
                        ...
                        <div class="table-responsive" id="tableContainer">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge modern-badge badge-primary">{{ $k->no_perjanjian_pihak_1 }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="kantor-info">
                                            @if($k->kantor)
                                                <span class="badge modern-badge badge-kantor">{{ $k->kantor->kode_kantor }}</span>
                                                <br><small class="text-muted">{{ $k->kantor->nama_kantor }}</small>
                                            @else
                                    ...
                                    <td>
                                        <small style="color: #64748b;">
                                            {{ \Carbon\Carbon::parse($k->tanggal_mulai)->format('d/m/Y') }} -<br>
                                            {{ \Carbon\Carbon::parse($k->tanggal_selesai)->format('d/m/Y') }}
                                        </small>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-success">Rp {{ number_format($k->nilai_kontrak, 0, ',', '.') }}</span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-{{ $k->status_perjanjian == 'Baru' ? 'success' : 'warning' }}">
                                            {{ $k->status_perjanjian }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-{{ $k->status == 'Aktif' ? 'success' : ($k->status == 'Tidak Aktif' ? 'warning' : 'danger') }}">
                                            {{ $k->status }}
                                        </span>
                                    </td>
                                    <td>
                                        ...
                                        <div class="action-buttons">
                                            <a href="{{ route('kontrak.show', $k->id) }}" class="btn btn-modern btn-info btn-sm" title="Lihat Detail">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            @if(($actor && $actor->role === 'super_admin') || ($actor && in_array($actor->role, ['admin_regional','staf']) && (int)$actor->kantor_id === (int)$rowKantorId))
                                            <a href="{{ route('kontrak.edit', $k->id) }}" class="btn btn-modern btn-warning btn-sm" title="Edit">
                                            ...
                                            <form action="{{ route('kontrak.destroy', $k->id) }}" method="POST" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-modern btn-danger btn-sm" 
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus kontrak ini?')" title="Hapus">
                                ...
                                @empty
                                <tr>
                                    <td colspan="{{ (Auth::guard('admin')->user()->role ?? '') === 'super_admin' ? '11' : '10' }}" class="text-center text-muted empty-state">
                                        <div class="empty-content">
                                            <i class="fas fa-file-contract"></i>
                                            <p>Belum ada data kontrak. Silakan tambah data kontrak baru.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
