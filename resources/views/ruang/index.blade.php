...
@section('page-subtitle', 'Kelola data ruang kantor PLN Icon Plus')

@section('page-actions')
    <a href="{{ route('ruang.create') }}" class="btn btn-modern btn-primary">
                                    ...
                                    <form id="filter-form" method="GET">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select modern-select" name="lantai" id="lantai-filter">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select modern-select" name="status_ruang" id="status-filter">
                        ...
                        <div class="table-responsive" id="tableContainer">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td>{{ $loop->iteration + ($ruang->currentPage() - 1) * $ruang->perPage() }}</td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="badge modern-badge badge-lantai">Lantai {{ $r->lantai->nomor_lantai ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <div class="gedung-info">
                                            <strong>{{ $r->lantai->gedung->nama_gedung ?? 'N/A' }}</strong>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="kantor-info">
                                            @if($r->lantai && $r->lantai->gedung && $r->lantai->gedung->kantor)
                                                <span class="badge modern-badge badge-kantor">{{ $r->lantai->gedung->kantor->kode_kantor }}</span>
                                                <br><small class="text-muted">{{ $r->lantai->gedung->kantor->nama_kantor }}</small>
                                            @else
                                    ...
                                    <td>
                                        <span class="badge modern-badge badge-bidang">{{ $r->bidang->nama_bidang ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-kapasitas">{{ $r->kapasitas }} orang</span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge {{ $r->status_ruang == 'tersedia' ? 'badge-success' : ($r->status_ruang == 'terisi' ? 'badge-warning' : 'badge-danger') }}">
                                    ...
                                    <td>
                                        ...
                                        <div class="action-buttons">
                                            <a href="{{ route('ruang.show', $r->id) }}" class="btn btn-modern btn-info btn-sm" title="Lihat Detail">
                                            ...
                                            <a href="{{ route('ruang.edit', $r->id) }}" class="btn btn-modern btn-warning btn-sm" title="Edit">
                                            ...
                                            <form action="{{ route('ruang.destroy', $r->id) }}" method="POST" class="d-inline">
                                                ...
                                                <button type="submit" class="btn btn-modern btn-danger btn-sm" 
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus ruang ini?')" title="Hapus">
                                ...
                                <tr>
                                    <td colspan="{{ (Auth::guard('admin')->user()->role ?? '') === 'super_admin' ? '10' : '9' }}" class="text-center text-muted empty-state">
                                        <div class="empty-content">
                                            <i class="fas fa-door-open"></i>
                                            <p>Belum ada data ruang. Silakan tambah data ruang baru.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (tableContainer) {
        ...
        // Drag scroll functionality - click and drag to scroll
        let isDown = false;
        let startX;
        let scrollLeft;
        ...
        tableContainer.addEventListener('mousedown', function(e) {
            ...
            // Only enable drag if there's horizontal scroll available
            if (tableContainer.scrollWidth <= tableContainer.clientWidth) {
                return;
            }
        ...
        tableContainer.addEventListener('mousemove', function(e) {
            if (!isDown) {
                // Continue with auto-scroll near edges logic
                return;
            }
