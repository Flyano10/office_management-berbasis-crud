...
@section('page-subtitle', 'Kelola data okupansi kantor PLN Icon Plus')

@section('page-actions')
    <a href="{{ route('okupansi.create') }}" class="btn btn-modern btn-primary">
                                    ...
                                    <form id="filter-form" method="GET">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select modern-select" name="ruang" id="ruang-filter">
                                                    <option value="">Semua Ruang</option>
                                                    @foreach($ruang ?? [] as $r)
                                                        <option value="{{ $r->id }}" {{ request('ruang') == $r->id ? 'selected' : '' }}>
                                                            {{ $r->nama_ruang }} - Lantai {{ $r->lantai->nomor_lantai ?? 'N/A' }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-3">
                                                ...
                                                <select class="form-select modern-select" name="bidang" id="bidang-filter">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="date" class="form-control modern-input" name="tanggal_okupansi_dari" value="{{ request('tanggal_okupansi_dari') }}">
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">Tanggal Sampai</label>
                                                <input type="date" class="form-control modern-input" name="tanggal_okupansi_sampai" value="{{ request('tanggal_okupansi_sampai') }}">
                                            </div>
                                        </div>
                        ...
                        <div class="table-responsive" id="tableContainer">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    @if((Auth::guard('admin')->user()->role ?? '') === 'super_admin')
                                    <td>
                                        <input type="checkbox" class="form-check-input modern-checkbox item-checkbox" value="{{ $o->id }}">
                                    </td>
                                    @endif
                                    ...
                                    <td>
                                        <span class="badge modern-badge badge-lantai">Lantai {{ $o->ruang->lantai->nomor_lantai ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <div class="gedung-info">
                                            <strong>{{ $o->ruang->lantai->gedung->nama_gedung ?? 'N/A' }}</strong>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="kantor-info">
                                            @if($o->ruang && $o->ruang->lantai && $o->ruang->lantai->gedung && $o->ruang->lantai->gedung->kantor)
                                                <span class="badge modern-badge badge-kantor">{{ $o->ruang->lantai->gedung->kantor->kode_kantor }}</span>
                                                <br><small class="text-muted">{{ $o->ruang->lantai->gedung->kantor->nama_kantor }}</small>
                                            @else
                                    ...
                                    <td>
                                        <span class="badge modern-badge badge-bidang">{{ $o->bidang->nama_bidang ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-pegawai">{{ $o->jml_pegawai_organik }}</span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-pegawai">{{ $o->jml_pegawai_tad }}</span>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-pegawai">{{ $o->jml_pegawai_kontrak ?? 0 }}</span>
                                    </td>
                                    <td>
                                        <strong style="color: var(--pln-blue);">{{ $o->total_pegawai }}</strong>
                                    </td>
                                    <td>
                                        <span class="badge modern-badge badge-kapasitas">{{ $o->ruang->kapasitas ?? 'N/A' }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <small style="color: #64748b;">{{ \Carbon\Carbon::parse($o->tanggal_okupansi)->format('d/m/Y') }}</small>
                                    </td>
                                    <td>
                                        @php($rowKantorId = $o->ruang->lantai->gedung->kantor->id ?? $o->ruang->lantai->gedung->kantor_id ?? null)
                                        @php($rowBidangId = $o->bidang_id ?? null)
                                        <div class="action-buttons">
                                            <a href="{{ route('okupansi.show', $o->id) }}" class="btn btn-modern btn-info btn-sm" title="Lihat Detail">
                                            ...
                                            <a href="{{ route('okupansi.edit', $o->id) }}" class="btn btn-modern btn-warning btn-sm" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('okupansi.destroy', $o->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-modern btn-danger btn-sm" 
                                                        onclick="return confirm('Apakah Anda yakin ingin menghapus okupansi ini?')" title="Hapus">
                                ...
                                @empty
                                <tr>
                                    <td colspan="{{ (Auth::guard('admin')->user()->role ?? '') === 'super_admin' ? '15' : '14' }}" class="text-center text-muted empty-state">
                                        <div class="empty-content">
                                            <i class="fas fa-chart-pie"></i>
                                            <p>Belum ada data okupansi. Silakan tambah data okupansi baru.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
